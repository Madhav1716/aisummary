...
export default function HomeScreen() {
  ...
  const handleUpload = async () => {
    try {
      ...
      const response = await fetch('http://192.168.29.221:3000/extract-text', {
        method: 'POST',
        body: formData,
        headers: {
      ...
      try {
        ...
        await AsyncStorage.setItem('summaries', JSON.stringify([...summaries, newSummary]));
    ...
    switch (featureId) {
      ...
      case 'fast':
        setComingSoonAlert({
          ...
          message: 'We\'re working on making our summaries even faster. Stay tuned for updates!',
          icon: 'lightning-bolt'
        });
        break;
      case 'ai':
        setComingSoonAlert({
          ...
          message: 'We\'re working on bringing you more advanced AI capabilities. Stay tuned for updates!',
          icon: 'brain'
        });
        break;
      case 'formats':
        setComingSoonAlert({
          ...
          message: 'We\'re working on supporting more document formats. Stay tuned for updates!',
          icon: 'file-document'
        });
        break;
      case 'chat':
  ...
  return (
    ...
    >
      ...
      >
        ...
        <TouchableOpacity
          style={[styles.uploadCard, { backgroundColor: isDark ? '#2a2a2a' : '#ffffff' }]}
          ...
          <BlurView intensity={80} style={styles.uploadContent}>
            {isLoading ? (
              ...
              <>
                ...
                />
                <Text style={[styles.uploadText, { color: isDark ? '#ffffff' : '#000000' }]}>
                  {selectedFile ? 'Upload Another PDF' : 'Upload PDF'}
                </Text>
                <Text style={[styles.uploadSubtext, { color: isDark ? '#a0a0a0' : '#666666' }]}>
        ...
        {summary && (
          <View style={[styles.summaryCard, { backgroundColor: isDark ? '#2a2a2a' : '#ffffff' }]}>
        ...
        <View style={styles.featuresContainer}>
          ...
          <View style={styles.featuresGrid}>
            {features.map((feature) => (
              <TouchableOpacity
                key={feature.id}
                style={[styles.featureCard, { backgroundColor: isDark ? '#2a2a2a' : '#ffffff' }]}
              ...
              >
                ...
                <Text style={[styles.featureDescription, { color: isDark ? '#a0a0a0' : '#666666' }]}>
                  {feature.description}
                </Text>
              </TouchableOpacity>
            ))}
      ...
      <ComingSoonAlert
        ...
        onClose={() => setComingSoonAlert(prev => ({ ...prev, visible: false }))}
